...
load m_Labels.mat
result = zeros(12*11, length(Labels));
...
label_files_dir = ''
registration_results_dir = cell(12, 12)
for from = 1:12
	for to = 1:12
		if (from == to)
			continue
...
label_images = cell(12, 1);
for i = 1:12
	label_images{i} = load_nii(strcat(label_files_dir, 'm', num2str(i), '.nii'));
end
