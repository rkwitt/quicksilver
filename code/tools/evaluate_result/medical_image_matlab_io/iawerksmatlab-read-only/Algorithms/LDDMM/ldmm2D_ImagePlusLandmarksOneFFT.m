...
if displayPlotOverview
  ...
  figHandles = [figHandle_Overview figHandle_Mean];
...
fprintf(' DONE (%g sec)\n',cputime-t);
memUsageImage = ...
  whos('I','J0T','Ihat','v','JTt','dPhitT','J0t','gJ0t','bLinvb','gE');
...
for k=1:maxIter
  ...
  %  vIhatPJPDiffs(:,:,[1 end],:) = 0;
    ...
    for tm = 1:size(bJPJPDiffs,ndims(bJPJPDiffs))
      fprintf('%d',tm);
      vJPJPDiffs(:,:,:,tm) = greensFunction(squeeze(bJPJPDiffs(:,:,:,tm)+bIhatPJPDiffs(:,:,:,tm)),...
					 operatorType,[alpha_pts beta_pts gamma_pts]); 
    end
    fprintf(' DONE (%g sec)\n',cputime-t);
    if dispMinMaxL2
