...
% don's use sigma because it is a built in function
sig = 0.08;
...
t = cputime;
I = zeros([dims(1) dims(2) dims(3) M],dtype);
for m=1:M
  fprintf('%d',m);
  I(:,:,:,m) = makeTestImage([dims(1) dims(2) dims(3)],'sphere',...
    [mindim*(m/M+1)/8, imageNoiseSigma, imageSmoothingSigma],dtype);
...
fprintf('v');
v = zeros([d dims(1) dims(2) dims(3) N M],dtype);
%%%%%%%%%  V,X,Y,Z,t,m  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
fprintf(',JTt');
JTt    = zeros([dims(1) dims(2) dims(3) N],dtype);
% det of Jacobian of h-field mapping \Omega at t to T
fprintf(',dPhitT');
dPhitT = zeros([dims(1) dims(2) dims(3) N],dtype);
% deformed image from time 0 to each time t
fprintf(',J0t');
J0t    = zeros([dims(1) dims(2) dims(3) N+1],dtype);
% (spatial) gradient of J0t images
fprintf(',gJ0t');
gJ0t   = zeros([d dims(1) dims(2) dims(3) N+1],dtype); 
% body force
fprintf(',b');
%b      = zeros([d s s s N],dtype); 
% regularized body force
fprintf(',Linvb');
bLinvb  = zeros([d dims(1) dims(2) dims(3) N],dtype); 
% gradient energy
fprintf(',gE');
gE     = zeros([d dims(1) dims(2) dims(3) N],dtype); 
fprintf(' DONE (%g sec)\n',cputime-t);
...
for k=1:maxIter
  for m=1:M
    ...
    bLinvb(3,:,:,:,:) = dPhitT.*(J0t(:,:,:,1:N)-JTt) .* squeeze(gJ0t(3,:,:,:,1:N));
    fprintf(' DONE (%g sec)\n',cputime-t);
    if dispMinMaxL2
      fprintf('NUM NAN = %g\n',sum(isnan(bLinvb(:))));
      fprintf('MAX = %g\n',max(bLinvb(:)));
    ...
    % add contribution due to points
