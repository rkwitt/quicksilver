...
def geodesic_shooting_diffOp(moving, target, m0, steps, mType, config):
	...
	diffOp.setAlpha(config['diffOpParams'][0])
	diffOp.setBeta(config['diffOpParams'][1])
	diffOp.setGamma(config['diffOpParams'][2])
	diffOp.setGrid(grid)
	...
	CAvmCommon.IntegrateGeodesic(m0,t,diffOp, mt, g, ginv,\
                                 ...
                                 Ninv=config['NIterForInverse'], integMethod = config['integMethod'])
