...
class Doxy2SWIG:    
    ...
    def __init__(self, src, include_function_definition=True, quiet=False):
        ...
        # pieces of text to go at top of file
        # jsp
        self.pieces = []
        self.pieces.append('\n// File: %s\n'%\
                           os.path.basename(f.name))

        # pieces of text to form the current docstring, points to
        # self.pieces if no element is being parsed (just to pick up
        # any orphaned pieces)
        # jsp
        self.curpieces = self.pieces

        # dictionary of lists of text pieces indexed by docstring name
        # jsp
        self.docpieces = {}
    ...
    def generate(self):
        ...
        data is stored in `self.pieces` and `self.docpieces`.

        """
        self.parse(self.xmldoc)

        # clean any pieces that have been put directly into pieces
        self.pieces = self.clean_pieces(self.pieces)

        # Here we need to take self.docpieces and add entries to
        # self.pieces
        for doc_el in self.docpieces:
            p = ['\n']
            p.append('%feature("docstring") ')
            p.append(doc_el)
            p.append('\n')
            p.append('"')
            p.append('================================\n')
            p.append('Doxygen Documentation\n')
            p.append('================================\n')
            p.extend(self.clean_pieces(self.docpieces[doc_el]))
            p.extend(['";', '\n'])
            self.pieces.extend(p)
    ...
    def add_text(self, value):
        """Adds text corresponding to `value` into `self.curpieces`."""
        if type(value) in (types.ListType, types.TupleType):
            self.curpieces.extend(value)
        else:
            self.curpieces.append(value)
    ...
    def generic_parse(self, node, pad=0):
        ...
        npiece = 0
        if pad:
            npiece = len(self.curpieces)
            if pad == 2:
                self.add_text('\n')                
        for n in node.childNodes:
            self.parse(n)
        if pad:
            if len(self.curpieces) > npiece:
                self.add_text('\n')
    ...
    def do_memberdef(self, node):
        ...
        if prot == 'public':
            ...
            if cdef_kind in ('file', 'namespace'):
                ...
                docstring_el = '%s::%s'%(cname, name)

            # get docstring pieces for this element, or create empty
            # list if no existing pieces
            if docstring_el not in self.docpieces:
                self.docpieces[docstring_el] = []
            self.curpieces = self.docpieces[docstring_el]

            if len(self.curpieces) > 0:
                self.add_text('\n')
                self.add_text('-------------------------------')
                self.add_text('\n')
    ...
    def do_sectiondef(self, node):
        kind = node.attributes['kind'].value
        if kind in ('public-func', 'public-static-func', 'func', 'user-defined', ''):
            self.generic_parse(node)
